    ...
    ++xIter)
  {
    ...
    for (kt_int32u angleIndex = 0; angleIndex < m_nAngles; angleIndex++) {
      ...
      if (m_doPenalize && (math::DoubleEqual(response, 0.0) == false)) {
        // =====================================================================
        // [ORIGINAL SLAM TOOLBOX CODE - PRESERVED]
        // Uncomment the block below to restore stock isotropic distance penalty:
        // =====================================================================
        /*
        ...
        */
        ...
        // =====================================================================
        // [MODULAR NARROW FOV OPTIMIZATION - CHANGE 4]
        // Parameter: enable_anisotropic_penalty
        // When TRUE: Applies 3x lateral weight along local sensor frame.
        // When FALSE: Applies standard isotropic Euclidean distance penalty.
        // =====================================================================
        // simple model (approximate Gaussian) to take odometry into account
        kt_double squaredDistance = squareX + squareY;
        if (m_pMapper->getParamEnableAnisotropicPenalty()) {
          kt_double heading = m_rSearchCenter.GetHeading();
          kt_double localX =  x * std::cos(heading) + y * std::sin(heading);
          kt_double localY = -x * std::sin(heading) + y * std::cos(heading);
          squaredDistance = (localX * localX) + 3.0 * (localY * localY);
        }
  ...
  kt_double searchAngleResolution, Matrix3 & rCovariance)
{
  ...
  if (norm > KT_TOLERANCE) {
    ...
    // =========================================================================
    // [ORIGINAL SLAM TOOLBOX CODE - PRESERVED]
    // Uncomment the block below to restore stock diagonal covariance matrix:
    // =========================================================================
    /*
    ...
    */
    ...
    // =========================================================================
    // [MODULAR NARROW FOV OPTIMIZATION - CHANGE 3]
    // Parameter: enable_coupled_covariance
    // When TRUE: Computes cross-terms (x-theta, y-theta) with Cauchy-Schwarz clamping.
    // When FALSE: Sets cross-terms to 0 (stock SLAM Toolbox behavior).
    // =========================================================================
    // increase variance for poorer responses
    kt_double multiplier = 1.0 / bestResponse;
    rCovariance(0, 0) = varianceXX * multiplier;
    ...
    if (m_pMapper->getParamEnableCoupledCovariance()) {
      kt_double varianceXTh = (accumulatedVarianceXY / norm) * multiplier;
      kt_double varianceYTh = (accumulatedVarianceYY / norm) * multiplier;

      // Cauchy-Schwarz bound to guarantee positive-definiteness for Ceres:
      kt_double maxCovXTh = 0.75 * std::sqrt(varianceXX * varianceTHTH);
      kt_double maxCovYTh = 0.75 * std::sqrt(varianceYY * varianceTHTH);

      rCovariance(0, 2) = std::clamp(varianceXTh, -maxCovXTh, maxCovXTh);
      rCovariance(2, 0) = rCovariance(0, 2);
      rCovariance(1, 2) = std::clamp(varianceYTh, -maxCovYTh, maxCovYTh);
      rCovariance(2, 1) = rCovariance(1, 2);
    } else {
      // Stock decoupled behavior
      rCovariance(0, 2) = 0.0;
      rCovariance(2, 0) = 0.0;
      rCovariance(1, 2) = 0.0;
      rCovariance(2, 1) = 0.0;
...
{
  // =========================================================================
  // [MODULAR NARROW FOV OPTIMIZATION - CHANGE 2]
  // Parameter: enable_narrow_fov_valid_points
  // When TRUE and sensor is non-360: Bypasses 360-degree winding polygon filter.
  // When FALSE (or 360 Lidar): Runs stock SLAM Toolbox winding filter.
  // =========================================================================
  if (m_pMapper->getParamEnableNarrowFovValidPoints()) {
    if (pScan->GetLaserRangeFinder() && !pScan->GetLaserRangeFinder()->GetIs360Laser()) {
      return pScan->GetPointReadings();
    }
  }
...
void Mapper::InitializeParameters()
{
  ...
  // =========================================================================
  // [MODULAR NARROW FOV OPTIMIZATION - PARAMETER INITIALIZATIONS]
  // =========================================================================
  m_pEnableNarrowFovValidPoints = new Parameter<kt_bool>(
    "EnableNarrowFovValidPoints",
    "Bypass 360-degree winding filter for narrow FoV lidars.",
    false, GetParameterManager());

  m_pEnableCoupledCovariance = new Parameter<kt_bool>(
    "EnableCoupledCovariance",
    "Enable cross-coupled covariance terms with Cauchy-Schwarz clamping.",
    false, GetParameterManager());

  m_pEnableAnisotropicPenalty = new Parameter<kt_bool>(
    "EnableAnisotropicPenalty",
    "Enable 3x lateral anisotropic penalty for narrow FoV lidars.",
    false, GetParameterManager());
...
// =========================================================================
// [MODULAR NARROW FOV OPTIMIZATION - NEW GETTERS]
// =========================================================================
bool Mapper::getParamEnableNarrowFovValidPoints()
{
  return static_cast<bool>(m_pEnableNarrowFovValidPoints->GetValue());
}

bool Mapper::getParamEnableCoupledCovariance()
{
  return static_cast<bool>(m_pEnableCoupledCovariance->GetValue());
}

bool Mapper::getParamEnableAnisotropicPenalty()
{
  return static_cast<bool>(m_pEnableAnisotropicPenalty->GetValue());
}
...
// =========================================================================
// [MODULAR NARROW FOV OPTIMIZATION - NEW SETTERS]
// =========================================================================
void Mapper::setParamEnableNarrowFovValidPoints(bool b)
{
  m_pEnableNarrowFovValidPoints->SetValue((kt_bool)b);
}

void Mapper::setParamEnableCoupledCovariance(bool b)
{
  m_pEnableCoupledCovariance->SetValue((kt_bool)b);
}

void Mapper::setParamEnableAnisotropicPenalty(bool b)
{
  m_pEnableAnisotropicPenalty->SetValue((kt_bool)b);
}
