...
/*****************************************************************************/
{
  ...
  // =========================================================================
  // [MODULAR NARROW FOV OPTIMIZATION - ROS 2 PARAMETER DECLARATIONS]
  // =========================================================================
  bool enable_narrow_fov_valid_points = false;
  if (!node->has_parameter("enable_narrow_fov_valid_points")) {
    node->declare_parameter("enable_narrow_fov_valid_points", enable_narrow_fov_valid_points);
  }
  node->get_parameter("enable_narrow_fov_valid_points", enable_narrow_fov_valid_points);
  mapper_->setParamEnableNarrowFovValidPoints(enable_narrow_fov_valid_points);

  bool enable_coupled_covariance = false;
  if (!node->has_parameter("enable_coupled_covariance")) {
    node->declare_parameter("enable_coupled_covariance", enable_coupled_covariance);
  }
  node->get_parameter("enable_coupled_covariance", enable_coupled_covariance);
  mapper_->setParamEnableCoupledCovariance(enable_coupled_covariance);

  bool enable_anisotropic_penalty = false;
  if (!node->has_parameter("enable_anisotropic_penalty")) {
    node->declare_parameter("enable_anisotropic_penalty", enable_anisotropic_penalty);
  }
  node->get_parameter("enable_anisotropic_penalty", enable_anisotropic_penalty);
  mapper_->setParamEnableAnisotropicPenalty(enable_anisotropic_penalty);
