...
/*****************************************************************************/
{
  ...
  // =========================================================================
  // [MODULAR NARROW FOV OPTIMIZATIONS - NEW PARAMETER REGISTRATION]
  // - enable_heading_scan_intake: Change 1 (In-place spin scan intake)
  // - enable_synchronized_intake_thresholds: Change 5 (1.0x Karto threshold sync)
  // - enable_heading_radian_units: Change 6 (Heading units degrees-to-radians fix)
  // =========================================================================
  enable_heading_scan_intake_ = false;
  enable_heading_scan_intake_ = this->declare_parameter(
    "enable_heading_scan_intake", enable_heading_scan_intake_);

  enable_synchronized_intake_thresholds_ = false;
  enable_synchronized_intake_thresholds_ = this->declare_parameter(
    "enable_synchronized_intake_thresholds", enable_synchronized_intake_thresholds_);

  enable_heading_radian_units_ = false;
  enable_heading_radian_units_ = this->declare_parameter(
    "enable_heading_radian_units", enable_heading_radian_units_);
...
/*****************************************************************************/
{
  ...
  // Uncomment the block below to restore stock distance-only scan filtering:
  // =========================================================================
  /*
  ...
  */
  ...
  // =========================================================================
  // [MODULAR NARROW FOV & THRESHOLD OPTIMIZATIONS - CHANGES 1, 5, 6]
  // Parameters:
  // - enable_heading_scan_intake: Evaluates rotational changes for scan intake
  // - enable_synchronized_intake_thresholds: Uses 1.0x exact Karto thresholds
  //   instead of stock 0.8x pre-filter to eliminate quantization dead-zones.
  // - enable_heading_radian_units: Converts Karto's degree getter to radians
  //   so deltaHeading (radians) compares against matching angular units.
  // =========================================================================
  const double dist2 = last_pose.SquaredDistance(pose);
  const double threshold_mult = enable_synchronized_intake_thresholds_ ? 1.0 : 0.8;

  if (enable_heading_scan_intake_) {
    const double deltaHeading = std::fabs(
      karto::math::NormalizeAngle(pose.GetHeading() - last_pose.GetHeading()));

    // -------------------------------------------------------------------------
    // [ORIGINAL GETTER WITHOUT CONVERSION - PRESERVED]
    // const double min_heading = smapper_->getMapper()->getParamMinimumTravelHeading();
    // -------------------------------------------------------------------------
    const double raw_min_heading = smapper_->getMapper()->getParamMinimumTravelHeading();
    const double min_heading = enable_heading_radian_units_ ?
      karto::math::DegreesToRadians(raw_min_heading) : raw_min_heading;

    if ((dist2 < threshold_mult * min_dist2 && deltaHeading < threshold_mult * min_heading) || scan_ctr < 5) {
      return false;
    }
  } else {
    // Stock SLAM Toolbox fallback
    if (dist2 < threshold_mult * min_dist2 || scan_ctr < 5) {
      return false;
    }
  }
